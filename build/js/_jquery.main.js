...
initLazyload();
initLazyLoadTagTemplate();

//init init Lazy-Load Tag Template
function initLazyLoadTagTemplate() {
	var holder = document.getElementById('mixitup');
	var tmpl =  document.getElementById('tmpl')
	var tmplItem = jQuery(tmpl.content).find('.portfolio-item').slice(0, 11);;


	console.log(tmplItem)
	holder.appendChild(tmpl.content);
}
...
function initSmoothScrollToAnotherPage() {
	...
	if(myHash[1] != undefined){
		jQuery('html, body').animate(
			{scrollTop: jQuery(myHash).offset().top - 50}
			, 700);
