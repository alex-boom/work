...
get_header(); ?>
		...
		<div id="page-product">
			<main class="main">
				<section id="product" class="section-product">
					<div class="container">
						<div class="row edit-btn">
							...
							<div class="col-lg-2"><a href="/cooked-snails" class="btn btn-product active-link"><?php echo __('[:ru]готовые улитки[:fr]prêt l-got[:pl]gotowe[:it]pronti[:es]listo[:de]bereit[:en]ready'); ?></a></div>
				...
				<?php
					$idObj = get_category_by_slug('product-2');
					$id = $idObj->term_id; 
				...
				<!-- Modal form-1-->
				<div class="modal fade closes" id="myModal-4" role="dialog">
					<div class="modal-dialog modal-sm">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal">&times;</button>
								<h4 class="modal-title"><?php echo __('[:ru]УЛИТКА ПО-БУРГУНДСКИ[:fr]Grape<br>snail[:pl]Grape<br>ślimak[:it]Uva<br>lumaca[:es]Caracol<br>de la uva[:de]Druiven<br>slak[:en]Druiven<br>slak'); ?></h4>
							</div>
							<div class="modal-body">
								...
								<form class="form form-hidden">
									<div class="form-group">
										...
										<input type="hidden" name="formname" value="УЛИТКА ПО-БУРГУНДСКИ">
				...
				<!-- Modal form-1-->
				<!-- Modal form-2-->
				<div class="modal fade closes" id="myModal-5" role="dialog">
					<div class="modal-dialog modal-sm">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal">&times;</button>
								<h4 class="modal-title"><?php echo __('[:ru]УЛИТКА ПО-ЛИМУЗИНСКИ[:fr]Snail<br>Achatina[:pl]Ślimak<br>Achatina[:it]Lumaca<br>Achatina[:es]Caracol<br>Achatina[:de]Slak<br>Achatina[:en]Slak<br>Achatina'); ?></h4>
							</div>
							<div class="modal-body">
								<form class="form form-hidden">
									<div class="form-group">
										...
										<input type="text" class="form-control" name="userName" placeholder="<?php echo __('[:ru]ВАШЕ ИМЯ*:[:fr]Votre nom *:[:pl]Imię i nazwisko *:[:it]Il tuo nome *:[:es]Su nombre *:[:de]IHR NAME*:[:en]YOUR NAME*:'); ?>">
										<input type="text" class="form-control" name="phone" placeholder="<?php echo __('[:ru]ТЕЛЕФОН:[:fr]TÉLÉPHONE:[:pl]TELEFON:[:it]TELEFONO:[:es]TELÉFONO:[:de]TELEFON:[:en]PHONE:'); ?>">
										<input type="hidden" name="formname" value="УЛИТКА ПО-ЛИМУЗИНСКИ">
										<button class="btn-small"><span><?php echo __('[:ru]ЗАКАЗАТЬ[:fr]Commander[:pl]Na zamówienie[:it]Su ordinazione[:es]Para ordenar[:de]bestellen[:en]order'); ?></span></button>
									</div>
								</form>
							</div>
				...
				<!-- Modal form-2-->
				<!-- Modal form-3-->
				<div class="modal fade closes" id="myModal-6" role="dialog">
					<div class="modal-dialog modal-sm">
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal">&times;</button>
								<h4 class="modal-title"><?php echo __('[:ru]УЛИТКА ПО-СОМЬЕРСКИ[:fr]Snail Promotions[:pl]Slimak Specials[:it]Snail Speciali[:es]Caracol Especiales[:nl]Slak Specials'); ?></h4>
							</div>
							<div class="modal-body">
								<form class="form form-hidden">
									<div class="form-group">
										...
										<input type="text" class="form-control" name="userName" placeholder="<?php echo __('[:ru]ВАШЕ ИМЯ*:[:fr]Votre nom *:[:pl]Imię i nazwisko *:[:it]Il tuo nome *:[:es]Su nombre *:[:de]IHR NAME*:[:en]YOUR NAME*:'); ?>">
										<input type="text" class="form-control" name="phone" placeholder="<?php echo __('[:ru]ТЕЛЕФОН:[:fr]TÉLÉPHONE:[:pl]TELEFON:[:it]TELEFONO:[:es]TELÉFONO:[:de]TELEFON:[:en]PHONE:'); ?>">
										<input type="hidden" name="formname" value="УЛИТКА ПО-СОМЬЕРСКИ">
										<button class="btn-small"><span><?php echo __('[:ru]ЗАКАЗАТЬ[:fr]Commander[:pl]Na zamówienie[:it]Su ordinazione[:es]Para ordenar[:de]bestellen[:en]order'); ?></span></button>
									</div>
								</form>
